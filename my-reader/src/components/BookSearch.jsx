...
function ListPicker({ book, lists, onClose }) {
  ...
  return (
    <>
      ...
      <div className="absolute right-0 top-full mt-1.5 z-20 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden min-w-48">
        {added ? (
          <p className="px-4 py-3 text-xs font-medium text-green-600">
            Added to "{added}" ✓
          </p>
        ) : (
          <>
    ...
    <div className="w-full max-w-2xl mx-auto">
      <div className="relative">
        <input
          ...
          autoFocus
          className="w-full px-4 py-3 pr-10 rounded-xl border border-gray-200 bg-white text-sm text-gray-900 placeholder-gray-400 shadow-sm focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent"
        />
        {loading && (
      ...
      {loading && results.length === 0 && (
        <ul className="mt-3 flex flex-col gap-2">
          {Array.from({ length: 5 }).map((_, i) => (
            <li key={i} className="flex items-center gap-4 bg-white border border-gray-100 rounded-xl p-3 shadow-sm">
              ...
              <div className="w-20 h-7 bg-gray-200 rounded-lg animate-pulse shrink-0" />
            </li>
          ))}
        </ul>
      )}
            ...
            >
              ...
              <div className="relative shrink-0">
                <button
                  ...
                  className="text-xs font-semibold text-white bg-gray-800 hover:bg-gray-700 rounded-lg px-3 py-1.5 transition-colors"
                >
                  + Add to list
                </button>
      ...
      {!loading && query.trim() && results.length === 0 && !error && (
        <p className="mt-4 text-sm text-gray-400 text-center">No results for "{query.trim()}"</p>
      )}
    </div>
  );
}
